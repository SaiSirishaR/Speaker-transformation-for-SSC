...
def get_spk_world_feats(spk_fold_path, mc_dir_train, mc_dir_test, sample_rate=16000):
    ...
    for wav_file in tqdm(train_paths):
        ...
        np.save(join(mc_dir_train, spk_name+'_' +wav_nam.split('.wav')[0]+'.npy'), normed_coded_sp, allow_pickle=False)
        #np.save(join(mc_dir_train, wav_nam.replace('.wav', spk_name+'_' +wav_nam+'.npy')), normed_coded_sp, allow_pickle=False)    

    for wav_file in tqdm(test_paths):
        wav_nam = basename(wav_file)
        f0, timeaxis, sp, ap, coded_sp = world_encode_wav(wav_file, fs=sample_rate)
        normed_coded_sp = normalize_coded_sp(coded_sp, coded_sps_mean, coded_sps_std)
        np.save(join(mc_dir_test, spk_name+'_' + wav_nam.split('.wav')[0]+'.npy'), normed_coded_sp, allow_pickle=False)
    return 0
...
if __name__ == '__main__':
    ...
    # The original wav in Google Wavnnet is 24K, first we want to resample to 16K
    resample_to_16k(origin_wavpath, target_wavpath, num_workers=num_workers)

    # Testing 2 TTS vocies.
    speaker_used = ['c', 'e', 'f', 'g','h']
    speaker_used = ['wavenet_'+i for i in speaker_used]
